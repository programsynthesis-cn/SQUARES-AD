...
class IOEncoder(nn.Module):
    ...
    def forward(self, seq, seq_lens, hidden_state=None, attention_outputs=None, attention_padding_mask=None):
        ...
        print('239 ', attention_padding_mask)
        ...
        print('245 ', attention_padding_mask)
        if attention_outputs is not None:
            ...
            print('248 ', attention_padding_mask)
            sorted_attention_outputs = attention_outputs.clone()[sorted_seq_lens_ind, :, :]
            print('250 ',attention_padding_mask[0])
            print(len(attention_padding_mask))
            # a = np.array(attention_padding_mask,dtype=int)
            # print(a)
            # attention_padding_mask = torch.Tensor(a)
            # torch.Tensor(attention_padding_mask)
            attention_padding_mask = attention_padding_mask[0]
            # hidden_state = hidden_size[0]
...
# 在波束搜索过程中包含波束的必要信息。为了进一步提高效率，我们还#保留预测的最后一个令牌。当尝试使用新令牌扩展波束时，我们确保#令牌组合是合法的，并且可以在DSL中产生有效的语句。
class Beam(object):
    ...
    def extend(self, token, log_prob, state, context):
        ...
        if not new_beam.extend_last_statement(token):
            print('model2 409 ',token)
            return None
        else:
            print('model2 412 ', token)
            return new_beam
